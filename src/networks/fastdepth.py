...
class FastDepth(nn.Module):
    def __init__(self,args, kernel_size=5):
    ...
    def forward(self,x):
        ...
        x= F.interpolate(self.conv1(x), scale_factor=2, mode='nearest')
        x= F.interpolate(self.conv2(x), scale_factor=2, mode='nearest')
        x = F.interpolate(x, layer3.shape[-2:], mode='nearest')
        x = x+layer3
        self.intermediate = x
        x= F.interpolate(self.conv3(x), layer2.shape[-2:], mode='nearest') + layer2
        x= F.interpolate(self.conv4(x), layer1.shape[-2:], mode='nearest') + layer1
        x = self.conv5(x)
        final = F.interpolate(x, scale_factor=2, mode='nearest')
...
class FastDepthV2(nn.Module):
    def __init__(self,args, kernel_size=5):
        ...
        self.decoder.output = nn.Sequential(
                                        nn.Conv2d(32, 1, kernel_size=1, bias=False),
                                        nn.Sigmoid()
                                    )
    def forward(self,x):
        ...
        return self.decoder.output(x) * self.args.max_depth
